...
1. `task-orchestrator` 处理的是**复杂、动态任务**，节点内容不可预先枚举。
2. 因为节点内容不可预知，**结果验收也不可被预先写死成固定业务规则**。
3. 因此，系统能稳定提供的是：
   - 动态声明完成契约的协议
   - 通用检查原语
   - 结构化完成证据
...
4. verifier 的职责是：
   ...
   - 发现明显缺口
   - 生成结构化证据
   - 帮助区分 `passed / partial / needs_review`
5. verifier **不** 负责：
   - 对复杂开放任务做最终真值判断
   - 对商业判断、策略判断、创意质量做机械定论
   - 让系统对不确定任务产生虚假的确定性

一句话：

> verifier 是辅助证据层，不是复杂动态任务的最终裁判器。
